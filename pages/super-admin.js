...
function SuperAdminDashboard() {
  ...
  const handleProvisionClient = async (e) => {
    ...
    try {
      // 1. Create client organization entry
      const newOrg = {
    ...
    <>
      ...
      <div style={{ backgroundColor: '#0f172a', minHeight: '100vh', color: '#f8fafc', fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 24px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
            <div>
              <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: '30px', fontWeight: 800, margin: '0 0 6px', color: '#ffffff' }}>
                Client Subscription & Access Control
              </h1>
              <p style={{ margin: 0, color: '#94a3b8', fontSize: '14px' }}>
                Provision client organizations, set 7-day trials, extend subscriptions, and enforce agent seat limits.
              </p>
            </div>
          ...
          {toastMessage && (
            <div style={{ backgroundColor: '#1e293b', border: '1px solid #22c55e', color: '#4ade80', padding: '12px 20px', borderRadius: '10px', marginBottom: '24px', fontWeight: 600, fontSize: '14px' }}>
              {toastMessage}
            </div>
          )}

          {/* Client Organizations Table */}
          <Card title="Client Organizations Directory" subtitle="Manage client trial periods, subscription status, and feature limits" style={{ backgroundColor: '#1e293b', border: '1px solid #334155', color: '#f8fafc' }} headerStyle={{ borderBottom: '1px solid #334155' }}>
            {isLoading ? (
              <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>Loading client directory...</div>
            ) : organizations.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>No client organizations onboarded yet. Click "+ Onboard New Client" to start!</div>
            ) : (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px', textAlign: 'left' }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid #334155', color: '#94a3b8', fontSize: '12px', textTransform: 'uppercase' }}>
                      ...
                      <th style={{ padding: '12px 16px' }}>Agent Seats</th>
                    ...
                    {organizations.map(org => {
                      ...
                      return (
                        <tr key={org.id} style={{ borderBottom: '1px solid #334155' }}>
                          ...
                          <td style={{ padding: '16px' }}>
                            <span style={{ backgroundColor: '#0f172a', border: '1px solid #475569', padding: '4px 10px', borderRadius: '6px', fontSize: '12px', fontWeight: 600, textTransform: 'uppercase', color: '#38bdf8' }}>
                          ...
                          <td style={{ padding: '16px', fontSize: '13px' }}>
                            {org.status === 'trial' ? (
                              <span style={{ color: trialDaysLeft > 0 ? '#4ade80' : '#f87171', fontWeight: 600 }}>
                                ⏳ {trialDaysLeft > 0 ? `${trialDaysLeft} day${trialDaysLeft === 1 ? '' : 's'} left` : 'Expired'}
                              </span>
                            ) : (
                              <span style={{ color: '#94a3b8' }}>Paid Subscription</span>
                            )}
                          </td>
          ...
          <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
            <div style={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '16px', padding: '32px', width: '100%', maxWidth: '480px', color: '#ffffff' }}>
              ...
              <form onSubmit={handleProvisionClient} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: '#cbd5e1', marginBottom: '6px' }}>Company Name</label>
                  <input
                    type="text"
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: '#cbd5e1', marginBottom: '6px' }}>Owner Name</label>
                  <input
                    type="text"
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: '#cbd5e1', marginBottom: '6px' }}>Owner Email</label>
                  <input
                    type="email"
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <div>
                    <label style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: '#cbd5e1', marginBottom: '6px' }}>Plan Tier</label>
                    <select
                      value={newPlanTier}
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: '#cbd5e1', marginBottom: '6px' }}>Max Agent Seats</label>
                    <input
                      type="number"
                      value={newMaxAgents}
                ...
                <div style={{ display: 'flex', gap: '12px', marginTop: '16px' }}>
                  <button
                    ...
                    style={{ flex: 1, padding: '12px', backgroundColor: '#334155', color: '#cbd5e1', border: 'none', borderRadius: '8px', fontWeight: 600, cursor: 'pointer' }}
                  >
                    Cancel
                  </button>
                  <button
