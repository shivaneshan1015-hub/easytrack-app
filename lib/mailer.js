...
export function buildInvoiceHtml({ invoiceSettings, transaction, items }) {
  ...
  const rows = (items || []).map((line, i) => `
    <tr style="background:${i % 2 === 0 ? '#ffffff' : '#f8fafc'};border-bottom:1px solid #e2e8f0;">
      ...
      <td style="padding:10px 8px;text-align:right;color:#475569;font-size:13px;white-space:nowrap;">₹${parseFloat(line.products?.unit_price || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
